...
import novaIconUrl from '../../assets/nova-agent-icon.png';
import { t } from '../../i18n';
import { appStatus } from '../../state/config';

export const LegacyLogo: Component<{ class?: string }> = (props) => (
  <svg
...
export const Logo: Component<{ class?: string }> = (props) => (
  <span class={props.class ?? 'inline-flex min-w-0 items-center gap-2.5'}>
    <img class="h-9 w-9 shrink-0 object-contain" src={novaIconUrl} alt="" aria-hidden="true" />
    <span class="hidden min-w-0 sm:block">
      <span class="block truncate text-[0.86rem] font-semibold text-[var(--color-text-primary)]">
        Nova
      </span>
      <span class="block truncate text-[0.64rem] text-[var(--color-text-muted)]">
        ESP32 Agent
      </span>
    </span>
  </span>
